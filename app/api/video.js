...
const { getRandomInt, sleep } = utils

const router = new Router({
    prefix: '/api/video'
})

router.get('/menu/index', async (ctx) => {
    ctx.response.status = 200
    if (ctx.query.wait) {
        const t = ctx.query.wait
        await sleep(t)
    } else {
        await sleep(1000)
    }
...
router.get('/index', async (ctx) => {
    const { topCategory, subCategory, order, limit, page, year, region } = ctx.query
...
function generateRandomActorsArray() {
...
function generateRandomSubCategories(topCategory) {
    let num = getRandomInt(1, 5)
    let subCategories = []
    for (let i = 0; i < num; i++) {
        let top = topCategory[getRandomInt(0, topCategory.length - 1)]
        if (menuTree[top] && menuTree[top].length) {
            let sub = menuTree[top][getRandomInt(0, menuTree[top].length - 1)]
...
function generateRandomTopCategories() {
...
function generateRandomCategoryTree() {
    let res = {}
    let num = getRandomInt(1, 2)
    for (let i = 0; i < num; i++) {
        let top = topCategoryValues[getRandomInt(0, topCategoryValues.length - 1)]
...
function getVideoArray(limit, topCategory, subCategory = '', order = '') {
    let videos = []
