...
if (lng === "zh_cn") {
  assignLocale[lng].push(
    getLangFileMessage(
      import.meta.glob("./pages/zh-cn/**/*.ts", { eager: true }),
      "zh-cn"
    )
  );
} else if (lng === "en") {
  ...
  for (let path in mList) {
    if (mList[path].default) {
      ...
      let pathName = path.slice(
        path.lastIndexOf(locale) + (locale.length + 1),
        path.lastIndexOf(".")
      );
      if (pathName.indexOf("/") > 0) {
        ...
        if (pathNameTmp.length == 2) {
          ...
          msg[pathNameTmp[0]][pathNameTmp[1]] = handleMsglist(
            mList[path].default
          );
        } else if (pathNameTmp.length == 3) {
          if (msg[pathNameTmp[0]] === undefined) msg[pathNameTmp[0]] = [];
          if (msg[pathNameTmp[0]][pathNameTmp[1]] === undefined)
            msg[pathNameTmp[0]][pathNameTmp[1]] = [];
          msg[pathNameTmp[0]][pathNameTmp[1]][pathNameTmp[2]] = handleMsglist(
            mList[path].default
          );
        } else {
